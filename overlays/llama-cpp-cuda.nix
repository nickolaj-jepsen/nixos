...
#
# Pinned ahead of nixpkgs (b10797, Sep 4): Qwen3.8 MTP speculative
# decoding matured and --reasoning-effort merged (PR #26941) on Aug 14, one day
# after b10425. nixpkgs is still pinned to v0.3.0 (Aug 25), which trails this
# pin by hundreds of commits. Drop the overrideAttrs once nixpkgs catches up
# past b10797.
{
  inputs,
  lib,
      ...
      in
        (pkgs.llama-cpp.override {cudaSupport = true;}).overrideAttrs (_old: {
          ...
          src = pkgs.fetchFromGitHub {
            ...
            repo = "llama.cpp";
            tag = "b10797";
            hash = "sha256-4+6LMCQK+TJWrmhw+dnh0pYnNAUVSHsGisG5CZPGmjE=";
          };
        });
    };
}
