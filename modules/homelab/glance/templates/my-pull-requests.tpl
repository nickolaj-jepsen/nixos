...
{{ else }}
  <ul class="list list-gap-10 collapsible-container" data-collapse-after="5">
    {{ range $i, $pr := $items }}
      ...
      {{ $reviewCount := $pr.Int "reviewRequests.totalCount" }}
      <li{{ if ge $i 5 }} class="collapsible-item" style="animation-delay: {{ mul (sub $i 5) 20 }}ms;"{{ end }}>
        <div style="display: flex; gap: 8px; align-items: flex-start;">
          {{ if $state }}<a href="{{ $url }}/checks" style="flex-shrink: 0; font-size: 1.2em; line-height: 1; text-decoration: none;">{{ if eq $state "SUCCESS" }}✅{{ else if eq $state "FAILURE" }}❌{{ else if eq $state "ERROR" }}⚠️{{ else if eq $state "PENDING" }}🔄{{ else }}⏳{{ end }}</a>{{ end }}
          <div style="min-width: 0; flex: 1;">
            ...
            <div style="font-size: 0.85em; margin-top: 2px;" class="color-subdue">
              ...
              · <span {{ $pr.String "updatedAt" | parseTime "rfc3339" | toRelativeTime }}></span>
              {{ if gt $reviewCount 0 }}· <a href="{{ $url }}" class="color-subdue">👀 {{ $reviewCount }}</a>{{ end }}
