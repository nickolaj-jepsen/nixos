...
<div class="linear-overview-grid">
  <div class="widget-content-frame linear-overview-box">
    <a href="https://linear.app/my-issues" class="size-h4 color-highlight" style="display: block; margin-bottom: 10px;">Issues · {{ len $items }}</a>
    {{ if eq (len $items) 0 }}
    ...
    {{ range $g, $group := sortByFloat "state.position" "desc" (unique "state.name" $items) }}
      ...
      <div class="size-h6 color-subdue" style="text-transform: uppercase; letter-spacing: 0.05em;{{ if gt $g 0 }} margin-top: 15px;{{ end }} margin-bottom: 8px;">{{ $name }}</div>
        ...
        {{ range $k := 5 }}
          ...
          {{ range $issue := $items }}
            {{ if and (eq ($issue.String "state.name") $name) (eq ($issue.Int "priority") $p) }}
              ...
              <li{{ if ge $j 5 }} class="collapsible-item" style="animation-delay: {{ mul (sub $j 5) 20 }}ms;"{{ end }}>
