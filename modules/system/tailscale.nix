{
  flake.modules.nixos.tailscale = {
    ...
    # WSL rides the Windows host's tailscaled; a second node here just fights it
    # (autoconnect re-runs `tailscale up` and fails, leaking the auth key to the journal).
  ...
  # not the sandboxed Mac App Store one). No auth-key hook: log in once in the GUI.
